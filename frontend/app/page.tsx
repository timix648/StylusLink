  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        ...
        // 🕒 NEW: Get Current Time (Seconds)
        const now = Math.floor(Date.now() / 1000);
        // 🕒 NEW: Check Expiry from the data details
        // We handle the string conversion just to be safe
        const expiry = data.details ? Number(data.details.expiresAt) : 0;
        const isExpired = expiry > 0 && now > expiry;



        if (data.claimed || data.reclaimed || isExpired) {
            setIsClaimed(true);
            setClaimStep('claimed_already');
        } else {
            // Only set to 'quest' if we aren't already processing a mobile handoff
            if (claimStep === 'loading' && !urlProof) setClaimStep('quest');
        }
      } catch (e) { console.error(e); }
    };

checkStatus();
    // INJECT: Lower interval to 1000ms (1s) for faster invalidation
    const interval = setInterval(checkStatus, 1000);

    // B. Handle Mobile Handoff (User scanned QR)
    if (urlProof && claimStep === 'loading') {
        setProofToken(urlProof);
        // If on mobile, jump straight to biometrics
        if (/iPhone|iPad|iPod|Android/i.test(navigator.userAgent)) {
            setClaimStep('bio_check');
            // Auto-trigger bio after a moment
            setTimeout(() => handleBiometrics(), 800);
        } else {
            setClaimStep('method'); 
        }
    }
